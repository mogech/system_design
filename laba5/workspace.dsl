workspace {
    model {
        ...
        cacheSystem  = softwareSystem "Система кешировнаия" "External"
        ...
        socialNetwork = softwareSystem "Социальная сеть" {
            ...
            userService = container "Сервис пользователей" "Python, FastAPI" {
                ...
                -> cacheSystem "Читает/пишет кеш пользователей" "Redis"
            }
            ...
            mongo = container "MongoDB" "MongoDB 5.0"
            redis = container "Redis" "Redis 7.0"
        ...
        apiGateway -> userService "Перенаправляет /register, /login, /users, /user/{username}" "HTTP"
        apiGateway -> wallService "Перенаправляет /posts" "HTTP"
        userService -> database "Читает/записывает пользователей" "PostgreSQL"
        wallService -> mongo "Читает/записывает записи" "MongoDB"
    ...
    views {
        ...
        dynamic socialNetwork "CreatePost" "Создание записи" {
            ...
            wallService -> mongo "Сохраняет запись"
            autoLayout
        }
        dynamic socialNetwork "UserLookup" "Запрос информации о пользователе" {
            user -> webApp "Просматривает профиль"
            webApp -> apiGateway "GET /user/{username}"
            apiGateway -> userService "Передаёт запрос"
            userService -> cacheSystem "Проверяет кеш"
            userService -> database "Если нет в кеше — читает из БД"
            userService -> cacheSystem "Обновляет кеш"
            autoLayout
        }
        styles {
            element "Person" {
